...
for k=1:noframes
    ...
    % Matched Filter
    filtered_rx_signal = matched_filter(noiseFrame(2,:), os_factor, 6); % 6 is a good value for the one-sided RRC length (i.e. the filter has 13 taps in total)
    ...
    rxsymbols(k,:) = exp(-1j*theta) * correct_samples;
